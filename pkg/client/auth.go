...
const (
	...
	XENCRIPTALGORITHM = "X-ENCRIPT-ALGORITHM"
...
func (a *auth) Login() error {
	...
	r.SetHostURL(host).
		...
		SetHeader(XAuthUserType, "0").
		SetHeader(XENCRIPTALGORITHM, "1")
...
func (a *auth) Logout() error {
	host := a.client.GetHost()
	r := common.NewHttpClient()
	r.SetHostURL(host).SetHeader(XAuthToken, string(a.client.GetSession()))
